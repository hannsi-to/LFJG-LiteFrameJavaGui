...
public class CefBrowserOsr extends CefBrowser_N implements CefRenderHandler {
    ...
    private static int getDndAction(int mask) {
        ...
        if ((mask & CefDragData.DragOperations.DRAG_OPERATION_COPY) == CefDragData.DragOperations.DRAG_OPERATION_COPY) {
            action = DnDConstants.ACTION_COPY;
        } else if ((mask & CefDragData.DragOperations.DRAG_OPERATION_MOVE) == CefDragData.DragOperations.DRAG_OPERATION_MOVE) {
            action = DnDConstants.ACTION_MOVE;
        } else if ((mask & CefDragData.DragOperations.DRAG_OPERATION_LINK) == CefDragData.DragOperations.DRAG_OPERATION_LINK) {
    ...
    protected CefBrowser_N createDevToolsBrowser(CefClient client, String url, CefRequestContext context, CefBrowser_N parent, Point inspectAt) {
        return new CefBrowserOsr(client, url, isTransparent_, context, this, inspectAt, null);
    }
    ...
    private void createGLCanvas() {
                    ...
                    if (OS.isMacintosh() && System.getProperty("java.runtime.version").startsWith("1.8")) {
                        ...
                        try {
                            if (scaleFactorAccessor == null) {
                                scaleFactorAccessor = getClass().getClassLoader().loadClass("sun.awt.CGraphicsDevice").getDeclaredMethod("getScaleFactor");
                            }
                            ...
                            if (factor instanceof Integer) {
                                ...
                                scaleFactor_ = 1.0;
                                 ...
                                 NoSuchMethodException | SecurityException | ClassNotFoundException exc) {
                            scaleFactor_ = 1.0;
                        }
                    } else {
        ...
        canvas_.addMouseWheelListener(this::sendMouseWheelEvent);
    ...
    public void onPaint(CefBrowser browser, boolean popup, Rectangle[] dirtyRects, ByteBuffer buffer, int width, int height) {
        ...
        context.release();
        SwingUtilities.invokeLater(() -> canvas_.display());
    ...
    public boolean onCursorChange(CefBrowser browser, final int cursorType) {
        SwingUtilities.invokeLater(() -> canvas_.setCursor(new Cursor(cursorType)));
    ...
    private void createBrowserIfRequired(boolean hasParent) {
        ...
        if (getNativeRef("CefBrowser") == 0) {
            if (getParentBrowser() != null) {
                createDevTools(getParentBrowser(), getClient(), windowHandle, true, isTransparent_, null, getInspectAt());
            } else {
    ...
    public CompletableFuture<BufferedImage> createScreenshot(boolean nativeResolution) {
        final Callable<BufferedImage> pixelGrabberCallable = getBufferedImageCallable(nativeResolution);
    ...
    private @NotNull Callable<BufferedImage> getBufferedImageCallable(boolean nativeResolution) {
        ...
        return () -> {
            ...
            if (!nativeResolution && scaleFactor_ != 1.0) {
                // HiDPI images should be resized down to "normal" levels
                BufferedImage resized = new BufferedImage((int) (screenshot.getWidth() / scaleFactor_), (int) (screenshot.getHeight() / scaleFactor_), BufferedImage.TYPE_INT_ARGB);
