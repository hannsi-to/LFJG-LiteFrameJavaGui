...
abstract class CefBrowser_N extends CefNativeAdapter implements CefBrowser {
    ...
    protected CefBrowser_N(CefClient client, String url, CefRequestContext context, CefBrowser_N parent, Point inspectAt, CefBrowserSettings settings) {
        client_ = client;
        url_ = url;
    ...
    CompletableFuture<Integer> executeDevToolsMethod(String method, String parametersAsJson) {
        ...
        N_ExecuteDevToolsMethod(method, parametersAsJson, generatedMessageId -> {
            if (generatedMessageId <= 0) {
                future.completeExceptionally(new DevToolsException(String.format("Failed to execute DevTools method %s", method)));
            } else {
    ...
    protected abstract CefBrowser_N createDevToolsBrowser(CefClient client, String url, CefRequestContext context, CefBrowser_N parent, Point inspectAt);

    /**
     * Create a new browser.
     */
    protected void createBrowser(CefClientHandler clientHandler, long windowHandle, String url, boolean osr, boolean transparent, Component canvas, CefRequestContext context) {
        if (getNativeRef("CefBrowser") == 0 && !isPending_) {
            try {
                N_CreateBrowser(clientHandler, windowHandle, url, osr, transparent, canvas, context, settings_);
    ...
    protected final void createDevTools(CefBrowser_N parent, CefClientHandler clientHandler, long windowHandle, boolean osr, boolean transparent, Component canvas, Point inspectAt) {
        if (getNativeRef("CefBrowser") == 0 && !isPending_) {
            try {
                isPending_ = N_CreateDevTools(parent, clientHandler, windowHandle, osr, transparent, canvas, inspectAt);
            } catch (UnsatisfiedLinkError err) {
                err.printStackTrace();
            }
        }
    ...
    private native boolean N_CreateBrowser(CefClientHandler clientHandler, long windowHandle, String url, boolean osr, boolean transparent, Component canvas, CefRequestContext context, CefBrowserSettings settings);

    private native boolean N_CreateDevTools(CefBrowser parent, CefClientHandler clientHandler, long windowHandle, boolean osr, boolean transparent, Component canvas, Point inspectAt);

    private native void N_ExecuteDevToolsMethod(String method, String parametersAsJson, IntCallback callback);

    private native CefRegistration N_AddDevToolsMessageObserver(CefDevToolsMessageObserver observer);

    private native long N_GetWindowHandle(long surfaceHandle);
    ...
    private native CefFrame N_GetFrameByIdentifier(String identifier);

    private native CefFrame N_GetFrameByName(String name);

    private native Vector<String> N_GetFrameIdentifiers();
    ...
    private native void N_GetSource(CefStringVisitor visitor);

    private native void N_GetText(CefStringVisitor visitor);

    private native void N_LoadRequest(CefRequest request);

    private native void N_LoadURL(String url);

    private native void N_ExecuteJavaScript(String code, String url, int line);

    private native String N_GetURL();

    private native void N_Close(boolean force);

    private native void N_SetFocus(boolean enable);

    private native void N_SetWindowVisibility(boolean visible);

    private native double N_GetZoomLevel();

    private native void N_SetZoomLevel(double zoomLevel);

    private native void N_RunFileDialog(FileDialogMode mode, String title, String defaultFilePath, Vector<String> acceptFilters, int selectedAcceptFilter, CefRunFileDialogCallback callback);

    private native void N_StartDownload(String url);

    private native void N_Print();

    private native void N_PrintToPDF(String path, CefPdfPrintSettings settings, CefPdfPrintCallback callback);

    private native void N_Find(String searchText, boolean forward, boolean matchCase, boolean findNext);

    private native void N_StopFinding(boolean clearSelection);

    private native void N_CloseDevTools();

    private native void N_ReplaceMisspelling(String word);

    private native void N_WasResized(int width, int height);

    private native void N_Invalidate();

    private native void N_SendKeyEvent(KeyEvent e);

    private native void N_SendMouseEvent(MouseEvent e);

    private native void N_SendMouseWheelEvent(MouseWheelEvent e);

    private native void N_DragTargetDragEnter(CefDragData dragData, Point pos, int modifiers, int allowed_ops);

    private native void N_DragTargetDragOver(Point pos, int modifiers, int allowed_ops);

    private native void N_DragTargetDragLeave();

    private native void N_DragTargetDrop(Point pos, int modifiers);

    private native void N_DragSourceEndedAt(Point pos, int operation);

    private native void N_DragSourceSystemDragEnded();

    private native void N_UpdateUI(Rectangle contentRect, Rectangle browserRect);

    private native void N_SetParent(long windowHandle, Component canvas);

    private native void N_NotifyMoveOrResizeStarted();

    private native void N_SetWindowlessFrameRate(int frameRate);

    private native void N_GetWindowlessFrameRate(IntCallback frameRateCallback);
