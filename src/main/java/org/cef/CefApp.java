...
public class CefApp extends CefAppHandlerAdapter {
    ...
    private static String getJcefLibPath() {
        ...
        String[] paths = library_path.split(File.pathSeparator);
        for (String path : paths) {
            File dir = new File(path);
            String[] found = dir.list((dir1, name) -> (name.equalsIgnoreCase("libjcef.dylib") || name.equalsIgnoreCase("libjcef.so") || name.equalsIgnoreCase("jcef.dll")));
     ...
     */
    private static String getCefFrameworkPath(String[] args) {
    ...
    protected final void handleBeforeTerminate() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                assert ((appHandler_ == null) ? this : appHandler_) instanceof CefAppHandler;
                CefAppHandler handler = appHandler_;
    ...
    private native boolean N_Initialize(CefAppHandler appHandler, CefSettings settings);
