...
public abstract class CefResponse implements AutoCloseable {
    ...
    public String toString() {
        ...
        returnValue.append("\n    HTTP/1.1 ").append(getStatus()).append(" ").append(getStatusText());
        returnValue.append("\n    Content-Type: ").append(getMimeType());

        Map<String, String> headerMap = new HashMap<>();
        getHeaderMap(headerMap);
        Set<Entry<String, String>> entrySet = headerMap.entrySet();
        for (Entry<String, String> entry : entrySet) {
            returnValue.append("    ").append(entry.getKey()).append("=").append(entry.getValue()).append("\n");
        }
