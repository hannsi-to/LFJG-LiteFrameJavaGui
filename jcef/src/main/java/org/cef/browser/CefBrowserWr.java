...
class CefBrowserWr extends CefBrowser_N {
    ...
    private CefBrowserWr(CefClient client, String url, CefRequestContext context, CefBrowserWr parent, Point inspectAt, CefBrowserSettings settings) {
    ...
    private static long getWindowHandle(Component component) {
        if (OS.isMacintosh()) {
            try {
                ...
                CefBrowserWindow browserWindow = (CefBrowserWindow) cls.getDeclaredConstructor().newInstance();
    ...
    protected CefBrowser_N createDevToolsBrowser(CefClient client, String url, CefRequestContext context, CefBrowser_N parent, Point inspectAt) {
        return new CefBrowserWr(client, url, context, this, inspectAt, null);
    }
    ...
    private void doUpdate() {
        ...
        Rectangle clipping = new Rectangle((int) (vr.getX() * scaleFactor_), (int) (vr.getY() * scaleFactor_), (int) (vr.getWidth() * scaleFactor_), (int) (vr.getHeight() * scaleFactor_));

        if (OS.isMacintosh()) {
            Point contentPos = getPoint(clipping);
    ...
    private @NotNull Point getPoint(Rectangle clipping) {
