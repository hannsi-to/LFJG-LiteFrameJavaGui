...
public abstract class CefPostDataElement implements AutoCloseable {
    ...
    public String toString(String mimeType) {
        ...
        if (getType() == Type.PDE_TYPE_BYTES) {
            int setBytes = 0;
            if (bytes != null) {
                setBytes = getBytes(bytes.length, bytes);
            }
            returnValue.append("    Content-Length: ").append(bytesCnt).append("\n");
            if (asText) {
                if (bytes != null) {
                ...
                for (int i = 0; i < setBytes; i++) {
                    ...
                    returnValue.append(String.format("%02X", bytes[i])).append(" ");
                }
            }
            returnValue.append("\n");
        } else if (getType() == Type.PDE_TYPE_FILE) {
            returnValue.append("\n    Bytes of file: ").append(getFile()).append("\n");
        }
     ...
     */
    public enum Type {
        PDE_TYPE_EMPTY, PDE_TYPE_BYTES, PDE_TYPE_FILE, PDF_TYPE_NUM_VALUES, // probably should be PDE_TYPE, but wrong in CEF too
    }
}
