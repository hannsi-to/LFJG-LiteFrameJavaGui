...
class CefPrintSettings_N extends CefPrintSettings implements CefNative {
    ...
    private native static CefPrintSettings_N N_Create();
    ...
    public void setPrinterPrintableArea(Dimension physical_size_device_units, Rectangle printable_area_device_units, boolean landscape_needs_flip) {
        try {
            N_SetPrinterPrintableArea(N_CefHandle, physical_size_device_units, printable_area_device_units, landscape_needs_flip);
        } catch (UnsatisfiedLinkError ule) {
            ule.printStackTrace();
        }
    }
    ...
    private native void N_SetPrinterPrintableArea(long self, Dimension physical_size_device_units, Rectangle printable_area_device_units, boolean landscape_needs_flip);
    ...
    private native void N_SetPageRanges(long self, Vector<CefPageRange> ranges);

    private native int N_GetPageRangesCount(long self);

    private native void N_GetPageRanges(long self, Vector<CefPageRange> ranges);

    private native void N_SetSelectionOnly(long self, boolean selection_only);
    ...
    private native void N_SetColorModel(long self, ColorModel model);

    private native ColorModel N_GetColorModel(long self);

    private native void N_SetCopies(long self, int copies);

    private native int N_GetCopies(long self);

    private native void N_SetDuplexMode(long self, DuplexMode mode);

    private native DuplexMode N_GetDuplexMode(long self);
}
