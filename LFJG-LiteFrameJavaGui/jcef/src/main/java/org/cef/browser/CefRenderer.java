...
class CefRenderer {
    ...
    protected Rectangle getPopupRectInWebView(Rectangle original_rect) {
        ...
        if (original_rect.x < 0) {
            original_rect.x = 0;
        }
        if (original_rect.y < 0) {
            original_rect.y = 0;
        }
        // if popup goes outside the view, try to reposition origin
        if (original_rect.x + original_rect.width > view_width_) {
            original_rect.x = view_width_ - original_rect.width;
        }
        if (original_rect.y + original_rect.height > view_height_) {
            original_rect.y = view_height_ - original_rect.height;
        }
